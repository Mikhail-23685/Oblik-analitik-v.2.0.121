...
namespace WS_с_интернетом
{
    public partial class ObjectNedv : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string id_apart;
            string myselectquery = "SELECT Max(id) FROM [dbo].[apartments]";
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            string myselectquery = "SELECT Max(id) FROM [dbo].[houses]";
            using (SqlDataAdapter dataAdapter = new SqlDataAdapter(myselectquery, SqlConnection))
            {
                DataTable table = new DataTable();
                dataAdapter.Fill(table);
            ...
            SqlCommand command1 = new SqlCommand("INSERT INTO [ObjectNedv] (id_clients,id_houses) VALUES (@id_clients,@id_houses)", SqlConnection);
            command1.Parameters.AddWithValue("id_clints", Program.id);
            command1.Parameters.AddWithValue("id_houses", id_apart);
            command1.ExecuteNonQuery();
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            string id_apart;
            string myselectquery = "SELECT Max(id) FROM [dbo].[lands]";
            using (SqlDataAdapter dataAdapter = new SqlDataAdapter(myselectquery, SqlConnection))
            {
                DataTable table = new DataTable();
                dataAdapter.Fill(table);
